...
if DEMAND_TYPE_MAP:
    ...
    rule build_tyndp_demand:
        input:
            demand=(
                rules.retrieve_tyndp_2026.output.demand_profiles
                if "retrieve_tyndp_2026" in dir(rules)
                else rules.retrieve_tyndp.output.demand_profiles
            ),
        output:
        ...
        benchmark:
            benchmarks(
                "performances/build_tyndp_demand_{demand_type}_{planning_horizons}"
            )
        wildcard_constraints:
        ...
        params:
            ...
            drop_leap_day=config_provider("enable", "drop_leap_day"),
            weather_scenarios=(
                config_provider("weather_scenarios_tyndp")
                if "weather_scenarios_tyndp" in config
                else {}
            ),
        script:
...
rule build_tyndp_gas_demand:
    input:
        supply_tool=(
            rules.retrieve_tyndp_2026.output.supply_tool
            if "retrieve_tyndp_2026" in dir(rules)
            else rules.retrieve_tyndp.output.supply_tool
        ),
