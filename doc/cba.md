...
- `data/custom_cba_generators_static.csv`: one row per generator, with `project_name`, `project_type`, `project_id`, `generator_name`, `carrier`, `bus`, `p_nom`, `marginal_cost`, `capital_cost` and `efficiency`. `project_type` and `project_id` identify the project the generator is grouped with: `project_type` is `t` for a transmission project or `s` for a storage project, and `project_id` is that project's ID, without the project_type (e.g. `project_type = t`, `project_id = 1500` for transmission project `t1500`). Entries whose `project_type` is neither `s` nor `t` are dropped with a warning, as are entries without a `project_id` or `generator_name`, or whose `bus` does not already exist in the network. Duplicate (`project_type`, `project_id`, `generator_name`) combinations are also dropped. Missing `marginal_cost`, `capital_cost` and `efficiency` default to `0`, `0` and `1` respectively.
