...
If you run the CBA workflow for the first time, you might not see a full DAG and instead only see a small number of steps listed in your DAG (including the ``clean_projects`` step). 
...
The checkpoint tells the workflow which CBA projects exist, which project IDs to run, and which method applies (TOOT/PINT).
...
So this short DAG is not reflective of the actual number of steps that will run once the checkpoint is passed.

Before the ``clean_projects`` step runs for the first time, Snakemake does not know the full list of project jobs it needs to create. 
...
To run the workflow up until the checkpoint, one can use the ``pixi run tyndp-checkpoint`` command, which will execute all steps up to and including the checkpoint. 

.. code-block:: console

    $ pixi run tyndp-checkpoint

After that, you can run the full workflow with ``pixi run tyndp-cba`` to execute all steps including those after the checkpoint.