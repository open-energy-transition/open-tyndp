...
.. list-table::
   ...
   * - Innovation Roadmap Details
     - Comparison with PyPSA-Eur / Open-TYNDP   
   * - 5.1 Dashboard: All graphs for the TYNDP 2026 report will be integrated into a reactive dashboard
   ...
   * - 5.2 Improvement of Reference Values
     ...
     - PyPSA-Eur recently updated its energy balances to JRC-IDEES-2021, switching the reference year to 2019. Open-TYNDP has refined electricity demand and biomass potentials to achieve an exact match with reference values.
       However, this challenge is shared by Open TYNDP. For the existing heating technologies in terms of capacities per country the `DG ENER mapping analysis <https://energy.ec.europa.eu/publications/mapping-and-analyses-current-and-future-2020-2030-heatingcooling-fuel-deployment-fossilrenewables-1_en>`_ is used, an update is needed.
       For district heating shares `Fraunhofer ISI data <https://fordatis.fraunhofer.de/handle/fordatis/341.5>`_. This data source is also used for geothermal heat potentials.
...
.. list-table::
   ...
   * - 8.3 Grid topology
     ...
     - Detailed H2 network topology is already implemented. 
       H2 flow are represented as a linear transport model. Line packing could be added easily.
       Physical flows including pressure drops would be harder, but possible.
   ...
   * - 8.4 Gas turbine usage and peaking unit utilisation 
     ...
     - Open TYNDP allows for endogenous retrofitting of CH4 pipeline capacity to H2.
       Endogenous retrofitting is also supported, to allow CH4 plants to operate instead with H2, or retrofitting of gas boilers to run with H2.
   * - 8.5 EV Modelling
     ...
     - PyPSA-Eur refined Vehicle-to-Grid (V2G) dispatch capacity and temperature-dependent energy demand correction factors for EVs. 
       Simultaneous charging/discharging is avoided in PyPSA by the use of small marginal costs.
   ...
   * - 8.9 Distinguish between hydrogen use as energy or feedstock 
     ...
     - H2 use for energy and feedstock purposes is already clearly distinguished in PyPSA-Eur
   ...
   * - 8.12 Higher granularity topology
     ...
     - 
       PyPSA-Eur was designed for highly resolved spatial and temporal modelling
       PyPSA-Eur supports multiple resource classes for wind and solar per region to improve accuracy at low spatial resolutions. 
       It also allows for behind-the-meter rooftop PV modeling.
         ...
         - different types of PV panels can be modelled on the distribution grid level (see `Rahdan et al. (2025) <https://www.nature.com/articles/s41467-025-61492-9>`_, 
           and `Rahdan et al. (2024) <https://www.sciencedirect.com/science/article/pii/S0306261924001041>`_
   ...
   * - 8.15 Check on remaining CO2 emissions in 2050
     - PyPSA-Eur models the full CO2 management (not only emissions, but also CCS, storage, CO2 networks). Remaining emissions can be checked in the final csvs and automated plots.
       One can add an extra constraint on top of the CO2 price, e.g. that CO2 emissions have to be net-zero in 2050
