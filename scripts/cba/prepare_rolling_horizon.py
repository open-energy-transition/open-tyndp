...
# ============================================================================
# EXPERIMENTAL Approach 1: Fix reservoir dispatch from PF (FAILED)
#
# Sets p_dispatch_set on hydro-reservoir StorageUnits to PF dispatch values.
# RESULT: Causes infeasibility in window 1. The RH optimizer in window 0
# keeps spill as a free variable, so it can drain the reservoir to 0 by the
# end of the window even though dispatch is fixed. Window 1 then inherits
# soc_initial ≈ 0 from the overlap, but the fixed PF dispatch requires
# soc >> 0 (e.g. PL00 needs ~544 MWh). Since soc >= 0 is a hard bound,
# the problem is infeasible.
# Fixing dispatch alone is insufficient — spill must also be controlled,
# which over-constrains the reservoir. Use Approach 2 instead.
# ============================================================================


def fix_reservoir_dispatch_from_pf(
    n: pypsa.Network,
    n_msv: pypsa.Network,
    carriers: list[str] | None = None,
) -> None:
    """
    Fix reservoir dispatch in rolling horizon to PF solution.

    Sets p_dispatch_set on StorageUnits whose carrier is in `carriers`,
    using the p_dispatch time series from the perfect foresight solve.
    The RH optimizer will treat these as fixed (no dispatch freedom).
    ...
    if n_msv.storage_units_t.p_dispatch.empty:
        logger.warning("PF network has no p_dispatch data, skipping dispatch fix")
        return

    sus_i = n.storage_units[n.storage_units.carrier.isin(carriers)].index
    common = sus_i.intersection(n_msv.storage_units_t.p_dispatch.columns)

    if len(common) == 0:
        logger.info(f"No StorageUnits with carriers {carriers} found, skipping dispatch fix")
        return

    pf_dispatch = n_msv.storage_units_t.p_dispatch[common]

    # Resample if snapshots differ (e.g. PF at different resolution)
    if not n.snapshots.equals(n_msv.snapshots):
        pf_dispatch = resample_msv_to_target(pf_dispatch, n.snapshots, method="ffill")

    n.storage_units_t.p_dispatch_set[common] = pf_dispatch

    stats = summarize_counts(n.storage_units.loc[common, "carrier"])
    total_twh = pf_dispatch.sum().sum() / 1e6
    logger.info(
        f"Fixed p_dispatch_set from PF for {len(common)} StorageUnits "
        f"(total dispatch: {total_twh:.2f} TWh):\n{stats}"
    )


# ============================================================================
# EXPERIMENTAL Approach 2: Fix reservoir SOC from PF
#
# Sets state_of_charge_set on hydro-reservoir StorageUnits to PF SOC values.
# The optimizer retains freedom to adjust dispatch and spill to meet the SOC
# trajectory, avoiding the spill-drain problem of Approach 1.
# ============================================================================


def fix_reservoir_soc_from_pf(
    n: pypsa.Network,
    n_msv: pypsa.Network,
    carriers: list[str] | None = None,
) -> None:
    """
    Fix reservoir SOC in rolling horizon to PF solution.

    Sets state_of_charge_set on StorageUnits whose carrier is in `carriers`,
    using the state_of_charge time series from the perfect foresight solve.
    The RH optimizer adjusts dispatch/spill to satisfy the fixed SOC.
    ...
    if n_msv.storage_units_t.state_of_charge.empty:
        logger.warning("PF network has no state_of_charge data, skipping SOC fix")
        return
    ...
    if len(common) == 0:
        logger.info(f"No StorageUnits with carriers {carriers} found, skipping SOC fix")
        return
    ...
    n.storage_units_t.state_of_charge_set[common] = pf_soc

    stats = summarize_counts(n.storage_units.loc[common, "carrier"])
    logger.info(
        f"Fixed state_of_charge_set from PF for {len(common)} StorageUnits:\n{stats}"
    )
