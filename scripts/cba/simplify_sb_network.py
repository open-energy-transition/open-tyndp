...
def disable_store_cyclicity(
    ...
    For CBA, long-term/seasonal storage should be free to have different start/end states,
    while short-term storage (e.g., batteries) and fuel stores should remain cyclic.
    ...
    seasonal_carriers : list[str], optional
        List of carrier names with seasonal behavior (e.g., ["H2 Store", "gas"]).
        These will have cyclicity disabled and will receive marginal storage values
        (MSV) during solving. Used for logging purposes.
    """
    ...
    if is_cyclic_carrier.any() and has_e_cyclic.any():
        ...
        if kept_cyclic.any():
            ...
            logger.info(f"Keeping e_cyclic=True for cyclic storage:\n{stats}")

    # Log seasonal carriers that will receive marginal storage values (MSV)
